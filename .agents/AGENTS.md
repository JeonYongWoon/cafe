...
* **에러 코드 체계화**: 실패 응답의 `error.code`에 들어갈 상수값은 반드시 도메인별 접두사를 명문화하여 설계합니다.
  ...
  * 메뉴 관련 에러 접두사: `MENU_` (예: `MENU_NOT_AVAILABLE`)
...
* **도메인 간 결합도 최소화**:
  ...
  * **퍼사드(Facade) 계층 예외 경계**: 다수의 바운디드 컨텍스트를 오케스트레이션해야 하는 복잡한 유스케이스(예: 주문 결제)에 한해, 퍼사드 계층은 타 도메인 서비스를 직접 주입받아 순서를 조율하는 유일한 예외 경계로 허용합니다. 단, 퍼사드는 반드시 해당 컨텍스트 하위의 `facade` 패키지에 위치해야 하며, 타 컨텍스트 서비스 호출은 조회/검증 목적으로만 최소화해야 합니다.
